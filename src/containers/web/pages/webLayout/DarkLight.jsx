...
function DarkLight() {
  ...
  const handleScreenReaderToggle = (event, selectedRate) => {
    event.stopPropagation();

    if (!("speechSynthesis" in window)) {
      Index.toast.error("Speech synthesis is NOT supported in this browser");
      return;
    }
    ...
    if (!isScreenReader) {
      dispatch(setIsScreenReader(true));
      dispatch(setScreenReaderRate(selectedRate));
      message = `Screen reader enabled. Reading ${selectedRate}`;
    } else if (screenReaderRate !== selectedRate) {
      dispatch(setScreenReaderRate(selectedRate));
      message = `Reading ${selectedRate}`;
    } else {
      dispatch(setIsScreenReader(false));
      dispatch(setScreenReaderRate(""));
      message = "Screen reader disabled";
    }
  ...
  return (
    <>
      <div className="fix-setting-icon">
        <div
          className={`accesi-detail-box ${
            active ? "cust-accesi-detail-box-active" : "cust-accesi-detail-box"
          }`}
        >
          <div className="accesi-cont-box">
            <div
              className="accesi-link custom-accesi-link"
            ...
            <div className="accesi-options-box">
              ...
              <div className="accesibility-btn-box">
                ...
                {["Normal", "Fast", "Slow"].map((rate) => (
                  <button
                    key={rate}
                    className={`accessibility-common-btn ${
                      isScreenReader && screenReaderRate === rate
                        ? "active"
                        : ""
                    }`}
                    onClick={(event) => handleScreenReaderToggle(event, rate)}
                  >
                    <span className="btn-icon-wrapper">
                      <img
                        src={PageIndex.Png.Voice_icon}
