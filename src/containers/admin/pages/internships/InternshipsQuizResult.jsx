...
import { getAllInternshipQuizResults } from "../../../../redux/services/AdminService";

export default function InternshipsQuizResult() {
  ...
  const columns = [
    ...
    { key: "title", label: "Week Title", limit: 30, width: "10%" },
  ...
  const actions = [
    {
      ...
      onClick: (row) =>
        navigate(`/admin/quiz-internships-result/view/${row?.id}`),
    },
  ];
  ...
  const fetchData = async (searchTerm, pageNo, dataPerPage) => {
    ...
    try {
      ...
      const res = await getAllInternshipQuizResults(data);
